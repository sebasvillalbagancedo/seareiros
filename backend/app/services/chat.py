...
# ── Helpers ───────────────────────────────────────────────────────
def get_membresia(chat_id: uuid.UUID, usuario_id: uuid.UUID, session: Session) -> MiembroChat | None:
...
def contar_admins(chat_id: uuid.UUID, session: Session) -> int:
...
def get_miembros_activos(chat_id: uuid.UUID, session: Session ) -> list[MiembroChat]:
...
def crear_miembro(chat_id: uuid.UUID, usuario_id: uuid.UUID, rol: str, session: Session) -> MiembroChat:
...
def get_chats_visibles(usuario: Usuario, session: Session ) -> list[Chat]:
    ...
    membresías = session.exec(
        select(MiembroChat.chat_id).where(
            MiembroChat.usuario_id == usuario.id,
            MiembroChat.estado == "activo"
        )
    ).all()
    ...
    return [
        c for c in chats
        if c.visibilidad == "visible" or c.id in ids_miembro
    ]
...
def crear_chat(datos: ChatCreate, usuario: Usuario, session: Session) -> Chat:
    ...
    crear_miembro(chat.id, usuario.id, "administrador", session)
...
def get_solicitud_pendiente(chat_id: uuid.UUID, usuario_id: uuid.UUID, session: Session) -> SolicitudChat | None:
...
def get_solicitudes_pendientes(chat_id: str, session: Session) -> list[tuple[SolicitudChat, Usuario]]:
...
def enviar_solicitud(chat: Chat, usuario: Usuario, session: Session) -> tuple[SolicitudChat | None, str]:
    """
    ...
    if chat.tipo_acceso == "libre":
        ...
        crear_miembro(chat.id, usuario.id, rol_entrada, session)
    else:
        solicitud = SolicitudChat(
            chat_id=chat.id,
            usuario_id=usuario.id,
...
def resolver_solicitud(solicitud: SolicitudChat,
    aceptar: bool,
    ...
    if aceptar:
        ...
        rol_entrada = "administrador" if usuario_solicitante and usuario_solicitante.rol == "administrador" else "miembro"
        miembro = crear_miembro(solicitud.chat_id, solicitud.usuario_id, rol_entrada, session)
...
def get_miembros(chat_id: str, session: Session) -> list[tuple[MiembroChat, Usuario]]:
    ...
    return session.exec(
        ...
        .where(
            MiembroChat.chat_id == uuid.UUID(chat_id),
            MiembroChat.estado  == "activo"
        )
        .order_by(Usuario.apellidos, Usuario.nombre)
    ).all()

def cambiar_rol(miembro: MiembroChat, nuevo_rol: str, session: Session) -> tuple[MiembroChat | None, str]:
    """
    ...
    if nuevo_rol == "miembro" and miembro.rol == "administrador":
        if contar_admins(miembro.chat_id, session) <= 1:
...
def dar_baja_miembro(miembro: MiembroChat, usuario_baja: Usuario, session: Session
    ...
    if miembro.rol == "administrador" and contar_admins(miembro.chat_id, session) <= 1:
...
def get_invitacion_pendiente(chat_id: uuid.UUID, usuario_destinatario: uuid.UUID, session: Session
) -> InvitacionChat | None:
...
def get_invitacion_rechazada(chat_id: uuid.UUID, usuario_destinatario: uuid.UUID, session: Session
) -> InvitacionChat | None:
...
def get_invitaciones_pendientes_usuario(usuario_id: uuid.UUID, session: Session
...
def enviar_invitacion(chat: Chat, usuario_destinatario: Usuario, admin: Usuario, session: Session
...
def resolver_invitacion(invitacion: InvitacionChat, aceptar: bool, session: Session
    ...
    if aceptar:
        ...
        rol_entrada = "administrador" if usuario_destinatario_obj and usuario_destinatario_obj.rol == "administrador" else "miembro"
        miembro = crear_miembro(
            invitacion.chat_id,
            invitacion.usuario_destinatario,
            rol_entrada,
            session
        )
...
def get_mensajes_chat(chat_id: str, usuario_id: uuid.UUID, session: Session
...
def get_mensajes_directos(usuario_id: uuid.UUID, otro_usuario_id: uuid.UUID,session: Session
) -> list[tuple[Mensaje, Usuario, MensajeDestinatario | None]]:
    """
...
def enviar_mensaje_directo(contenido: str, remitente: Usuario, destinatario: Usuario, session: Session) -> Mensaje:
    """
...
def enviar_mensaje_chat(contenido: str, remitente: Usuario, chat: Chat, session: Session) -> tuple[Mensaje | None, str]:
    """
    ...
    miembros = get_miembros_activos(chat.id, session)
...
def marcar_mensajes_leidos(mensaje_ids: list[uuid.UUID], usuario_id: uuid.UUID, session: Session) -> None:
    """
...
def get_historial_directo(usuario_id: uuid.UUID,session: Session) -> list[dict]:
    ...
    for interlocutor_id in interlocutores:
        ...
        ultimo = session.exec(
            select(Mensaje)
            .outerjoin(MensajeDestinatario, MensajeDestinatario.mensaje_id == Mensaje.id)
            .where(
                Mensaje.chat_id == None,
                or_(
                    and_(Mensaje.usuario_envio == usuario_id,
                         MensajeDestinatario.usuario_destinatario == interlocutor_id),
                    and_(Mensaje.usuario_envio == interlocutor_id,
                         MensajeDestinatario.usuario_destinatario == usuario_id)
                )
            )
    ...
    return sorted(resultado, key=lambda x: x["fecha_ultimo"] or datetime.min, reverse=True)
...
def get_historial_chats(usuario_id: uuid.UUID, session: Session) -> list[dict]:
    ...
    for membresia in membresías:
        ...
        ).one() if membresia.rol == "administrador" else 0
