...
from app.schemas.chat import (ChatCreate, ChatUpdate, ChatOutput,
    MiembroChatOutput, MiembroRolUpdate, SolicitudChatOutput,
    InvitacionChatCreate, InvitacionChatOutput, MensajeCreate, MensajeOutput,
    ConversacionDirectaOutput, ChatResumenOutput
)
from app.services.chat import (
    get_chats_visibles, get_chat, crear_chat, editar_chat,
    get_membresia, get_solicitud_pendiente, es_admin_chat,
    contar_miembros, get_miembros, get_solicitudes_pendientes,
    enviar_solicitud, resolver_solicitud, cambiar_rol, dar_baja_miembro,
    get_invitacion_pendiente, get_invitaciones_pendientes_usuario,
    enviar_invitacion, resolver_invitacion,
    get_mensajes_chat, get_mensajes_directos,
    enviar_mensaje_directo, enviar_mensaje_chat,
    marcar_mensajes_leidos, get_historial_directo, get_historial_chats,
)
...
def _solicitud_to_output(solicitud: SolicitudChat, usuario: Usuario) -> SolicitudChatOutput:
    ...
    return SolicitudChatOutput(
        ...
        usuario_resolucion=str(solicitud.usuario_resolucion)
                           if solicitud.usuario_resolucion else None,
    )

def _invitacion_to_output(invitacion: InvitacionChat, chat: Chat, destinatario: Usuario,invitador: Usuario,) -> InvitacionChatOutput:
...
def _mensaje_to_output(mensaje, usuario_remitente, destinatario_registro) -> MensajeOutput:
    ...
    return MensajeOutput(
        ...
        fecha_lectura=destinatario_registro.fecha_lectura
                      if destinatario_registro else None,
    )

# ── Endpoints de chats ───────────────────────────────────────────

@router.get("/chats", response_model=list[ChatOutput],
            summary="Listar chats visibles")
def get_chats_ep(
...
@router.post("/chats/{chat_id}/solicitudes",
...
@router.get("/chats/{chat_id}/solicitudes",
...
@router.patch("/chats/{chat_id}/solicitudes/{solicitud_id}",
              response_model=SolicitudChatOutput,
...
def get_miembros_ep(
    ...
    if not chat:
        raise HTTPException(status_code=404, detail="Chat no encontrado")
    if not get_membresia(chat.id, usuario.id, session) and usuario.rol != "administrador":
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN,
...
@router.patch("/chats/{chat_id}/miembros/{miembro_id}/rol",
...
def patch_miembro_rol_ep(
    ...
    if datos.rol not in ("miembro", "administrador"):
        raise HTTPException(status_code=400,
                            detail="El rol debe ser 'miembro' o 'administrador'")
...
@router.patch("/chats/{chat_id}/miembros/{miembro_id}/baja",
              response_model=MiembroChatOutput,
...
@router.post("/chats/{chat_id}/invitaciones",
...
@router.patch("/invitaciones/{invitacion_id}",
              response_model=InvitacionChatOutput,
...
def get_mensajes_chat_ep(
    ...
    if not get_membresia(chat.id, usuario.id, session):
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN,
                            detail="No eres miembro de este chat")
...
@router.get("/directos/{usuario_id}",
            response_model=list[MensajeOutput],
...
def post_mensaje_directo_ep(
    ...
    if not datos.usuario_destinatario:
        raise HTTPException(status_code=400,
                            detail="Debes indicar el usuario destinatario")
