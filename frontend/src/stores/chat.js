...
export const useChatStore = defineStore('chat', () => {
  ...
  const idsConversacionDirecta = computed(() => historialDirectos.value.map((d) => d.usuario_id))
  ...
  async function cambiarRol(chatId, miembroId, rol) {
    const { data } = await api.patch(`/chat/chats/${chatId}/miembros/${miembroId}/rol`, { rol })
  ...
  async function darBajaMiembro(chatId, miembroId) {
    const { data } = await api.patch(`/chat/chats/${chatId}/miembros/${miembroId}/baja`)
  ...
  async function resolverInvitacion(invitacionId, aceptar) {
    const { data } = await api.patch(`/chat/invitaciones/${invitacionId}?aceptar=${aceptar}`)
  ...
  async function enviarMensajeDirecto(usuarioDestinatarioId, contenido) {
    ...
    const idx = historialDirectos.value.findIndex((d) => d.usuario_id === usuarioDestinatarioId)
    if (idx !== -1) {
